...
from sample_datasets import (one_instance_training_dataset, same_label_training_dataset, example_training_dataset)
...
def test_example_dataset_returns_correct_split():
    ...
    assert split.split_value == 3

def test_axample_dataset_returns_correct_split_twice():
    (split, l_dataset, r_dataset) = find_split(example_training_dataset)
    split2 = find_split(l_dataset)
    assert split2.attribute == 1
    assert split2.split_value == 3
